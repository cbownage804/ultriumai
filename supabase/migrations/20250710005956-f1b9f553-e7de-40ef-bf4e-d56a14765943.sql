-- Remove all demo/sample data from the database (in correct order to handle foreign keys)

-- Remove event correlations first
DELETE FROM event_correlations WHERE primary_event_id IN (
  SELECT id FROM security_events WHERE user_id = '453c6d29-34db-4b1a-9f29-3ff7170ae765'
) OR related_event_id IN (
  SELECT id FROM security_events WHERE user_id = '453c6d29-34db-4b1a-9f29-3ff7170ae765'
);
...
-- Remove sample RMM data (in correct order)
