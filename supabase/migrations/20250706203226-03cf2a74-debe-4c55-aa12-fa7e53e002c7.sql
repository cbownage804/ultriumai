-- Add demo data for SafePass, SafeMail, SafeNet, and Security Center
...
INSERT INTO safepass_entries (vault_id, title, entry_type, url, notes, is_favorite, password_strength_score, is_compromised) VALUES
((SELECT id FROM safepass_vaults WHERE vault_name = 'Personal Vault' LIMIT 1), 'Gmail Account', 'login', 'https://gmail.com', 'Personal email account', true, 85, false),
((SELECT id FROM safepass_vaults WHERE vault_name = 'Personal Vault' LIMIT 1), 'Banking - Chase', 'banking', 'https://chase.com', 'Primary checking account', true, 92, false),
((SELECT id FROM safepass_vaults WHERE vault_name = 'Personal Vault' LIMIT 1), 'Netflix', 'streaming', 'https://netflix.com', 'Entertainment subscription', false, 65, false),
((SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'Office 365', 'work', 'https://office.com', 'Corporate email and documents', true, 78, false),
((SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'AWS Console', 'cloud', 'https://aws.amazon.com', 'Cloud infrastructure management', true, 88, false),
((SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'Old LinkedIn', 'social', 'https://linkedin.com', 'Professional networking - needs update', false, 35, true),
((SELECT id FROM safepass_vaults WHERE vault_name = 'Shared Team Vault' LIMIT 1), 'Team Slack', 'communication', 'https://slack.com', 'Team communication platform', false, 72, false);
...
((SELECT id FROM safenet_assets WHERE hostname = 'web-prod-01' LIMIT 1), 'CVE-2023-1234', 'Apache HTTP Server Buffer Overflow', 'Critical buffer overflow vulnerability in Apache HTTP Server', 'critical', 9.8, 'open', now() - interval '2 days', true, 'medium'),
((SELECT id FROM safenet_assets WHERE hostname = 'web-prod-01' LIMIT 1), 'CVE-2023-5678', 'SSL/TLS Configuration Issue', 'Weak SSL/TLS configuration allows downgrade attacks', 'high', 7.4, 'open', now() - interval '1 week', true, 'low'),
((SELECT id FROM safenet_assets WHERE hostname = 'db-prod-01' LIMIT 1), 'CVE-2023-9012', 'MySQL Privilege Escalation', 'Local privilege escalation in MySQL server', 'critical', 8.8, 'patching_scheduled', now() - interval '3 days', true, 'high'),
((SELECT id FROM safenet_assets WHERE hostname = 'db-prod-01' LIMIT 1), 'CVE-2023-3456', 'Database Access Control Bypass', 'Authentication bypass vulnerability', 'high', 8.1, 'open', now() - interval '5 days', true, 'medium'),
((SELECT id FROM safenet_assets WHERE hostname = 'ceo-laptop' LIMIT 1), 'CVE-2023-7890', 'Windows Kernel Elevation', 'Local privilege escalation through Windows kernel', 'high', 7.8, 'patched', now() - interval '1 week', true, 'low'),
((SELECT id FROM safenet_assets WHERE hostname = 'file-legacy-01' LIMIT 1), 'CVE-2019-1234', 'SMB Remote Code Execution', 'Critical RCE vulnerability in SMB protocol', 'critical', 10.0, 'open', now() - interval '2 weeks', true, 'high');
