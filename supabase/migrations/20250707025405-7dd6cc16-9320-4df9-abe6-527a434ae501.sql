-- Populate RMM Devices with sample data (corrected)
...
SELECT msp.id, client_data.client_name, client_data.contact_email, client_data.contact_phone, client_data.billing_address, client_data.status, client_data.monthly_revenue, client_data.device_count, client_data.created_at
FROM public.msps msp,
(VALUES 
  ('TechCorp Solutions', 'admin@techcorp.com', '+1-555-0123', '123 Business Ave, Tech City, TC 12345', 'active', 2500.00, 25, now() - interval '6 months'),
  ('Healthcare Partners', 'it@healthpartners.com', '+1-555-0456', '456 Medical Drive, Health City, HC 67890', 'active', 4200.00, 40, now() - interval '4 months'),
  ('Legal Associates', 'tech@legalassoc.com', '+1-555-0789', '789 Law Street, Legal Town, LT 13579', 'active', 1800.00, 15, now() - interval '8 months'),
  ('Retail Group', 'support@retailgroup.com', '+1-555-0321', '321 Store Plaza, Retail City, RC 24680', 'active', 3500.00, 35, now() - interval '3 months')
) AS client_data(client_name, contact_email, contact_phone, billing_address, status, monthly_revenue, device_count, created_at)
...
-- Sample document and email scans
