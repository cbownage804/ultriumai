-- Email Integration & Notifications Enhancement
...
CREATE POLICY "MSP owners can view all time entries for their tickets" ON public.time_entries
  FOR SELECT USING (ticket_id IN (
    SELECT ht.id FROM public.helpdesk_tickets ht
    JOIN public.msp_clients mc ON ht.client_id = mc.id
    WHERE mc.msp_id IN (SELECT id FROM public.msps WHERE user_id = auth.uid())
  ));
...
DECLARE
  ...
  msp_id_var UUID;
BEGIN
  -- Get the MSP ID for this ticket
  SELECT mc.msp_id INTO msp_id_var
