-- Add demo data for SafePass, SafeMail, SafeNet, and Security Center (Fixed with encrypted_data)
...
-- Insert demo SafePass entries (with all required fields)
...
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Personal Vault' LIMIT 1), 'Banking - Chase', 'banking', '{"username":"demo_encrypted_bank","password":"demo_encrypted_strong"}', 'https://chase.com', 'Primary checking account', true, 92, false),
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Personal Vault' LIMIT 1), 'Netflix', 'streaming', '{"username":"demo_user","password":"demo_weak_pass"}', 'https://netflix.com', 'Entertainment subscription', false, 65, false),
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'Office 365', 'work', '{"username":"demo_work_user","password":"demo_work_pass"}', 'https://office.com', 'Corporate email and documents', true, 78, false),
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'AWS Console', 'cloud', '{"username":"demo_aws_user","password":"demo_aws_strong"}', 'https://aws.amazon.com', 'Cloud infrastructure management', true, 88, false),
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'Old LinkedIn', 'social', '{"username":"demo_linkedin","password":"123456"}', 'https://linkedin.com', 'Professional networking - needs update', false, 35, true),
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Shared Team Vault' LIMIT 1), 'Team Slack', 'communication', '{"username":"team_slack","password":"demo_team_pass"}', 'https://slack.com', 'Team communication platform', false, 72, false);
