-- Migration: Enhance existing RMM tables for MVP functionality (simplified)
...
-- RLS policies for rmm_command_results
CREATE POLICY "Users can manage their own command results" ON public.rmm_command_results FOR ALL USING (user_id = auth.uid());
...
CREATE INDEX IF NOT EXISTS idx_rmm_command_results_user_id ON public.rmm_command_results(user_id);