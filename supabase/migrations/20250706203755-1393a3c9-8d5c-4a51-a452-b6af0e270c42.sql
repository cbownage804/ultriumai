-- Add demo data for SafePass, SafeMail, SafeNet, and Security Center (Fixed entry types)
...
-- Insert demo SafePass entries (with standard entry types)
...
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Personal Vault' LIMIT 1), 'Netflix', 'login', '{"username":"demo_user","password":"demo_weak_pass"}', 'https://netflix.com', 'Entertainment subscription', false, 65, false),
...
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Work Vault' LIMIT 1), 'AWS Console', 'login', '{"username":"demo_aws_user","password":"demo_aws_strong"}', 'https://aws.amazon.com', 'Cloud infrastructure management', true, 88, false),
...
('453c6d29-34db-4b1a-9f29-3ff7170ae765', (SELECT id FROM safepass_vaults WHERE vault_name = 'Shared Team Vault' LIMIT 1), 'Team Slack', 'login', '{"username":"team_slack","password":"demo_team_pass"}', 'https://slack.com', 'Team communication platform', false, 72, false);
