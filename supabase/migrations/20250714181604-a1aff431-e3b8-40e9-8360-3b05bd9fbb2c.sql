...
CREATE TABLE IF NOT EXISTS public.network_devices (
  ...
  scan_id UUID REFERENCES public.network_scans,
...
CREATE INDEX IF NOT EXISTS idx_network_scans_timestamp ON public.network_scans(scan_timestamp);
...
CREATE OR REPLACE FUNCTION update_network_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
FOR EACH ROW EXECUTE FUNCTION update_network_updated_at_column();
