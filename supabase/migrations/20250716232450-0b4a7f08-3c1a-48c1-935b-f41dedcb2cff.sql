-- Enhanced SafeNet Network Topology and Discovery Schema
...
-- Create SNMP monitoring data
CREATE TABLE IF NOT EXISTS public.safenet_snmp_data (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  device_id UUID REFERENCES public.safenet_devices(id) ON DELETE CASCADE,
  oid TEXT NOT NULL,
  oid_name TEXT,
  value_type TEXT, -- integer, string, counter, gauge, etc.
  value_raw TEXT,
  value_parsed JSONB,
  unit TEXT,
...
-- Create network monitoring alerts
CREATE TABLE IF NOT EXISTS public.safenet_monitoring_alerts (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  connector_id UUID REFERENCES public.safenet_connectors(id) ON DELETE CASCADE,
  device_id UUID REFERENCES public.safenet_devices(id) ON DELETE CASCADE,
  alert_type TEXT NOT NULL, -- device_down, high_utilization, security_event, etc.
...
ALTER TABLE public.safenet_snmp_data ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.safenet_monitoring_alerts ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can manage their own SNMP data" ON public.safenet_snmp_data
  FOR ALL USING (user_id = auth.uid());

CREATE POLICY "Users can manage their own monitoring alerts" ON public.safenet_monitoring_alerts
  FOR ALL USING (user_id = auth.uid());

-- Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_safenet_devices_user_id ON public.safenet_devices(user_id);
CREATE INDEX IF NOT EXISTS idx_safenet_devices_connector_id ON public.safenet_devices(connector_id);
...
CREATE INDEX IF NOT EXISTS idx_safenet_snmp_device_id ON public.safenet_snmp_data(device_id);
CREATE INDEX IF NOT EXISTS idx_safenet_snmp_collection_time ON public.safenet_snmp_data(collection_time);

CREATE INDEX IF NOT EXISTS idx_safenet_alerts_user_id ON public.safenet_monitoring_alerts(user_id);
CREATE INDEX IF NOT EXISTS idx_safenet_alerts_device_id ON public.safenet_monitoring_alerts(device_id);
CREATE INDEX IF NOT EXISTS idx_safenet_alerts_status ON public.safenet_monitoring_alerts(alert_status);

-- Create functions for topology analysis
CREATE OR REPLACE FUNCTION public.calculate_network_topology_metrics(p_user_id UUID, p_connector_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE user_id = p_user_id AND connector_id = p_connector_id;
  ...
  SELECT COUNT(DISTINCT network_segment) INTO network_segments
  FROM safenet_devices
  WHERE user_id = p_user_id AND connector_id = p_connector_id
    AND network_segment IS NOT NULL;
  
  -- Count critical devices
  SELECT COUNT(*) INTO critical_devices
  FROM safenet_devices
  WHERE user_id = p_user_id AND connector_id = p_connector_id
  ...
  FROM safenet_vulnerabilities v
  JOIN safenet_devices d ON v.device_id = d.id
  WHERE v.user_id = p_user_id AND d.connector_id = p_connector_id
    AND v.status = 'open';
  ...
  result := jsonb_build_object(
    ...
    'open_vulnerabilities', vulnerability_count
  );
...
-- Create function to update device relationships
CREATE OR REPLACE FUNCTION public.update_device_relationships()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  -- Update device count in network segments
  UPDATE safenet_network_segments
  SET device_count = (
    SELECT COUNT(*)
    FROM safenet_devices
    WHERE network_segment = safenet_network_segments.segment_name
      AND user_id = safenet_network_segments.user_id
      AND connector_id = safenet_network_segments.connector_id
  )
...
-- Create triggers
CREATE TRIGGER update_device_count_trigger
  AFTER INSERT OR UPDATE OR DELETE ON safenet_devices
  FOR EACH ROW
  EXECUTE FUNCTION update_device_relationships();

-- Create function for automated topology discovery
CREATE OR REPLACE FUNCTION public.discover_network_topology(p_user_id UUID, p_connector_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  result JSONB;
  gateway_devices UUID[];
  device_record RECORD;
BEGIN
  -- Find gateway devices (devices with default_gateway role or multiple interfaces)
  SELECT ARRAY_AGG(id) INTO gateway_devices
  FROM safenet_devices
  WHERE user_id = p_user_id 
    AND connector_id = p_connector_id
    AND (device_role LIKE '%gateway%' OR device_type = 'router');
  
  -- Create logical connections based on network topology
  FOR device_record IN
    SELECT d1.id as source_id, d2.id as target_id
    FROM safenet_devices d1
    CROSS JOIN safenet_devices d2
    WHERE d1.user_id = p_user_id 
      AND d1.connector_id = p_connector_id
      AND d2.user_id = p_user_id 
      AND d2.connector_id = p_connector_id
      AND d1.id != d2.id
      AND (d1.default_gateway = d2.ip_address OR d2.default_gateway = d1.ip_address)
  LOOP
    INSERT INTO safenet_topology (
      user_id, connector_id, source_device_id, target_device_id,
      connection_type, discovery_protocol
    )
    VALUES (
      p_user_id, p_connector_id, device_record.source_id, device_record.target_id,
      'logical', 'gateway_analysis'
    )
    ON CONFLICT (source_device_id, target_device_id, connection_type) DO NOTHING;
  END LOOP;
  
  result := jsonb_build_object('status', 'success', 'message', 'Topology discovery completed');
  RETURN result;
END;
$$;