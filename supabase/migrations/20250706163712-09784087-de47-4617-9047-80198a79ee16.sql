...
-- Create RLS policies
CREATE POLICY "Users can manage their device clipboard sync" ON public.rmm_clipboard_sync
  FOR ALL USING (device_id IN (
    SELECT id FROM public.rmm_endpoints WHERE user_id = auth.uid()
  ));

CREATE POLICY "Users can manage their device file transfers" ON public.rmm_file_transfers
  FOR ALL USING (device_id IN (
    SELECT id FROM public.rmm_endpoints WHERE user_id = auth.uid()
  ));

CREATE POLICY "Users can view their device session events" ON public.rmm_session_events
...
CREATE POLICY "Users can manage their device quarantine" ON public.safedoc_quarantine
  FOR ALL USING (device_id IN (
    SELECT id FROM public.rmm_endpoints WHERE user_id = auth.uid()
  ));
...
-- Create cleanup function for expired clipboard data
CREATE OR REPLACE FUNCTION cleanup_expired_clipboard_data()
RETURNS void AS $$
BEGIN
  DELETE FROM public.rmm_clipboard_sync 
  WHERE expires_at < now();
END;
$$ LANGUAGE plpgsql;

-- Create function to automatically cleanup old session events (keep last 30 days)
CREATE OR REPLACE FUNCTION cleanup_old_session_events()
RETURNS void AS $$
BEGIN
  DELETE FROM public.rmm_session_events 
