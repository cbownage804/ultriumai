...
CREATE TABLE public.sla_policies (
  ...
  response_hours INTEGER NOT NULL, -- Hours to first response
  resolution_hours INTEGER NOT NULL, -- Hours to resolution
  business_hours_only BOOLEAN DEFAULT true,
  escalation_enabled BOOLEAN DEFAULT true,
  escalation_hours INTEGER, -- When to escalate
  notification_intervals INTEGER[] DEFAULT '{1,4,24}', -- Hours before due to send reminders
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE POLICY "Users can manage their own SLA policies" ON public.sla_policies
FOR ALL USING (user_id = auth.uid());
...
FOR ALL USING (
  ticket_id IN (
    SELECT id FROM helpdesk_tickets WHERE user_id = auth.uid()
  )
);
...
CREATE INDEX idx_sla_policies_user_id ON public.sla_policies(user_id);
...
CREATE INDEX idx_ticket_templates_user_id ON public.ticket_templates(user_id);
