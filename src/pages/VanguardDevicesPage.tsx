...
export default function VanguardDevicesPage() {
  ...
  return (
    <div className="min-h-screen">
      {/* Page Header */}
      <div className="bg-slate-900/50 border-b border-cyan-500/20 px-6 py-4">
      ...
      {/* Content */}
      <div className="p-6">
        <div className="bg-slate-900/80 rounded-lg border border-cyan-500/20 shadow-lg backdrop-blur-sm">
          {/* Toolbar */}
          <div className="px-4 py-3 border-b border-cyan-500/10 flex items-center justify-between gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                ...
                className="pl-9 h-9 bg-slate-800/50 border-slate-700 text-white placeholder:text-slate-500 focus:border-cyan-500"
              />
            </div>

            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="text-slate-300 border-slate-700 hover:bg-cyan-500/10 hover:text-cyan-400 hover:border-cyan-500/30">
                Save view
              </Button>
              <Select defaultValue="default">
                <SelectTrigger className="w-[130px] h-9 border-slate-700 bg-slate-800/50 text-white">
              ...
              <Select value={selectedSite} onValueChange={setSelectedSite}>
                <SelectTrigger className="w-[120px] h-9 border-slate-700 bg-slate-800/50 text-white">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-cyan-500/10">
                ...
                <th className="text-left text-xs font-medium text-slate-400 px-4 py-3">Device name</th>
                <th className="text-center text-xs font-medium text-slate-400 px-2 py-3 w-12">AI</th>
                <th className="text-left text-xs font-medium text-slate-400 px-4 py-3">Availability</th>
                <th className="text-left text-xs font-medium text-slate-400 px-4 py-3">Device type</th>
                <th className="text-left text-xs font-medium text-slate-400 px-4 py-3">Site name</th>
                <th className="text-left text-xs font-medium text-slate-400 px-4 py-3">Last login</th>
                <th className="text-center text-xs font-medium text-slate-400 px-4 py-3">Remote access</th>
                <th className="text-center text-xs font-medium text-slate-400 px-4 py-3">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredDevices.map((device) => (
                <tr
                  key={device.id}
                  className="border-b border-slate-800/50 hover:bg-cyan-500/5 transition-colors cursor-pointer"
                ...
                >
                  ...
                  <td className="px-2 py-3 text-center">
                    {device.hasAI && (
                      <div className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-slate-800">
                  ...
                  <td className="px-4 py-3">
                    <a href="#" className="text-sm text-cyan-400 hover:text-cyan-300 hover:underline" onClick={(e) => e.stopPropagation()}>
                      {device.siteName}
                    </a>
                  </td>
                  <td className="px-4 py-3">
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="outline"
                            size="sm"
                            className="gap-1.5 text-cyan-400 border-cyan-500/30 hover:bg-cyan-500/10 hover:border-cyan-500/50"
                          >
                  ...
                  <td className="px-4 py-3 text-center" onClick={(e) => e.stopPropagation()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" size="sm" className="gap-1 border-slate-700 text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 hover:border-cyan-500/30">
                          Manage
                          <ChevronDown className="h-3 w-3" />
                        </Button>
                      </DropdownMenuTrigger>
