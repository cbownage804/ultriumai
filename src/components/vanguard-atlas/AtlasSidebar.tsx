/**
 * IT Glue-style vertical sidebar for Atlas documentation system
...
export interface AtlasSidebarItem {
  id: string;
  label: string;
  icon: LucideIcon;
  badge?: number;
...
export interface AtlasSidebarGroup {
  id: string;
  label: string;
  icon: LucideIcon;
...
interface AtlasSidebarProps {
  groups: AtlasSidebarGroup[];
  activeItem: string;
  onSelect: (itemId: string) => void;
}

export function AtlasSidebar({ groups, activeItem, onSelect }: AtlasSidebarProps) {
  return (
...
function SidebarGroup({ group, activeItem, onSelect }: { group: AtlasSidebarGroup; activeItem: string; onSelect: (id: string) => void }) {
    ...
    return (
      ...
      >
        ...
        {item.badge != null && item.badge > 0 && (
          <Badge variant="secondary" className="ml-auto h-5 px-1.5 text-[10px] bg-white/10 text-white/50">
            {item.badge}
          </Badge>
        )}
      </button>
          ...
          {group.items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge != null && item.badge > 0 && (
                  <Badge variant="secondary" className="ml-auto h-4 px-1 text-[10px] bg-amber-500/20 text-amber-400 border-0">
                    {item.badge}
                  </Badge>
                )}
              </button>
