...
export function VanguardNavigation() {
  ...
  const renderNavItem = (item: NavItem) => (
    <NavLink
      ...
      className={cn(
        ...
        isActive(item.path) && "bg-gradient-to-r from-cyan-500/25 to-cyan-500/10 text-cyan-400 border-l-2 border-cyan-400 shadow-[inset_0_0_20px_rgba(6,182,212,0.1)]"
      )}
    >
      ...
      {item.badge && (
        <span className="text-xs bg-gradient-to-r from-cyan-500 to-cyan-600 text-white px-1.5 py-0.5 rounded-full font-medium shadow-lg shadow-cyan-500/30">
          {item.badge}
        </span>
      )}
    </NavLink>
  ...
  return (
    <>
      ...
      {/* Sidebar - Pure Black Vanguard Theme with Vivid Accents */}
      <aside
        className={cn(
          "fixed left-0 top-0 h-full bg-black border-r border-cyan-500/30 z-40 transition-all duration-300 w-56 shadow-2xl shadow-cyan-500/5",
          isMobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        )}
      >
        <div className="flex flex-col h-full">
          {/* Logo Section - Vanguard Logo with Glow */}
          <div className="flex items-center justify-center px-4 py-4 border-b border-cyan-500/30 bg-gradient-to-b from-cyan-500/5 to-transparent">
            <img 
              src={vanguardLogo} 
              alt="Vanguard" 
              className="h-10 w-auto object-contain drop-shadow-[0_0_8px_rgba(6,182,212,0.3)]"
            />
          </div>
          ...
          {/* Footer */}
          <div className="p-4 border-t border-cyan-500/30 bg-gradient-to-t from-cyan-500/5 to-transparent">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <Shield className="h-3.5 w-3.5 text-cyan-400 drop-shadow-[0_0_4px_rgba(6,182,212,0.5)]" />
              <span>Powered by <span className="text-cyan-400/80">Ultrium</span></span>
            </div>
          </div>
        </div>
      </aside>
