...
export function BulkScriptExecution({ agents, onExecute }: BulkScriptExecutionProps) {
  ...
  const handleExecute = async () => {
    if (selectedDevices.length === 0 || !script.trim()) {
      toast.error('Select devices and enter a script');
      return;
    }
    ...
    const initialResults: ExecutionResult[] = selectedDevices.map(id => ({
      deviceId: id,
      deviceName: agents.find(a => a.id === id)?.device_name || 'Unknown',
      status: 'pending',
    }));
    setResults(initialResults);

    // Simulate execution with staggered results
    for (let i = 0; i < selectedDevices.length; i++) {
      ...
      setResults(prev => prev.map((r, idx) => {
        ...
        if (idx === i + 1) {
          return { ...r, status: 'running', startedAt: new Date() };
        }
        return r;
      }));
    }
                      ...
                      {filteredAgents.map((agent) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="font-medium text-sm">{agent.device_name}</div>
                            <div className="text-xs text-muted-foreground">{agent.os_type}</div>
                          </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button
                    ...
                    disabled={selectedDevices.length === 0 || !script.trim() || isExecuting}
                  >
                    {isExecuting ? (
                      <>
                      ...
                      <>
                        ...
                        Execute on {selectedDevices.length} Device{selectedDevices.length !== 1 ? 's' : ''}
                      </>
                    )}
                  </Button>
                </CardContent>
                  ...
                  {results.map((result) => (
                    <TableRow key={result.deviceId}>
                      ...
                      <TableCell className="max-w-[200px]">
                        {result.output && (
                          <pre className="text-xs text-muted-foreground truncate">{result.output}</pre>
                        )}
                        {result.error && (
