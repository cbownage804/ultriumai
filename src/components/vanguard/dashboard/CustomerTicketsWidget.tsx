      ...
      <CardContent className="space-y-1">
        {customers.length === 0 ? (
          ...
          customers.slice(0, 5).map((customer) => {
            ...
            return (
              <div key={customer.client_id} className="flex items-center justify-between py-2.5 hover:bg-cyan-500/10 px-2 rounded-lg cursor-pointer transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-xs font-bold border shadow-lg ${colors.bg} ${colors.text} ${colors.border}`}>
                    {customer.client_name.substring(0, 2).toUpperCase()}
                  </div>
                  <span className="text-sm text-slate-200">{customer.client_name}</span>
                </div>
                <span className="text-sm text-cyan-400 font-semibold bg-cyan-500/25 px-2.5 py-0.5 rounded border border-cyan-500/40">{customer.ticket_count}</span>
              </div>
            );
          })
        )}
