...
import { BarChart3, Users, TrendingUp, Activity, ArrowUpRight, ArrowDownRight, Monitor, Smartphone, Globe } from 'lucide-react';
import { AreaChart, Area, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Legend } from 'recharts';

// Mock data — replace with real queries from user_activity_feed
const dauData = Array.from({ length: 30 }, (_, i) => ({
  date: `Feb ${i + 1}`,
  dau: Math.floor(Math.random() * 200 + 80),
  mau: Math.floor(Math.random() * 50 + 400),
}));

const productAdoptionData = [
  { name: 'AI Studio', users7d: 142, users30d: 389, newActivations: 34, color: 'hsl(var(--primary))' },
  { name: 'Vanguard', users7d: 98, users30d: 267, newActivations: 21, color: 'hsl(180, 80%, 50%)' },
  { name: 'SafeSuite', users7d: 76, users30d: 198, newActivations: 15, color: 'hsl(280, 70%, 55%)' },
];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Platform usage metrics and user engagement</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Daily Active Users" value="184" change="+12%" positive icon={Users} />
        <KPICard title="Monthly Active Users" value="467" change="+8%" positive icon={TrendingUp} />
        <KPICard title="Avg Session Duration" value="14m 32s" change="+5%" positive icon={Activity} />
        <KPICard title="Activation Rate" value="49%" change="-2%" positive={false} icon={ArrowUpRight} />
      </div>
      ...
      <Tabs defaultValue="engagement" className="space-y-4">
              ...
              <Card key={product.name}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {Math.round((product.users7d / product.users30d) * 100)}% weekly retention
                  </p>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value="funnel">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Signup → Activation Funnel</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
...
function KPICard({ title, value, change, positive, icon: Icon }: { title: string; value: string; change: string; positive: boolean; icon: React.ElementType }) {
  return (
    <Card>
