...
/** Flatten tree into a visible-items list for virtualization */
interface FlatItem {
  node: TreeNode;
  depth: number;
}

function flattenTree(nodes: TreeNode[], expandedFolders: Set<string>, depth: number = 0): FlatItem[] {
...
const ITEM_HEIGHT = 24; // px per row
const OVERSCAN = 8;
...
function TreeItem({ 
  ...
  if (node.isFolder) {
    return (
      <button
        ...
        style={{ paddingLeft: `${depth * 12 + 6}px`, paddingRight: 6, height: ITEM_HEIGHT }}
      >
        {isExpanded ? (
  ...
  return (
    <div
      ...
      style={{ paddingLeft: `${depth * 12 + 20}px`, paddingRight: 4, height: ITEM_HEIGHT }}
...
export function ProjectFileTree({ files, activeFilePath, onSelectFile, modifiedPaths }: ProjectFileTreeProps) {
  ...
  // For small trees (<100 items), render normally without virtualization
  const useVirtualization = flatItems.length > 100;
  ...
  const endIdx = Math.min(flatItems.length, Math.ceil((scrollTop + containerHeight) / ITEM_HEIGHT) + OVERSCAN);
