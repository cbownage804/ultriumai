import { X, Bug, Check, AlertTriangle, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
export function ErrorTrackingPanel({ open, onClose, errors, stats, onResolve, onDelete, onInsertCode, onGenerateBoundary }: Props) {
  ...
  const trendColor = stats.trend === 'rising' ? 'text-red-400' : stats.trend === 'falling' ? 'text-emerald-400' : 'text-white/30';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#12121a] border border-white/[0.08] rounded-xl w-[650px] max-h-[80vh] flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {/* Alert banner */}
          {stats.alertThresholdExceeded && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20">
              <AlertTriangle className="h-4 w-4 text-red-400 shrink-0" />
              <span className="text-[11px] text-red-300">High error volume detected — {stats.unresolvedCount} unresolved errors need attention</span>
            </div>
          )}
          ...
          {/* Mini sparkline for errors by hour */}
          {stats.errorsByHour.length > 1 && (
            <div className="flex items-end gap-px h-8 px-1">
              {stats.errorsByHour.slice(-12).map((h, i) => {
                const max = Math.max(...stats.errorsByHour.slice(-12).map(x => x.count), 1);
                const pct = (h.count / max) * 100;
                return (
                  <div
                    key={i}
                    className="flex-1 rounded-t bg-red-400/40 min-h-[2px] transition-all"
                    style={{ height: `${Math.max(pct, 8)}%` }}
                    title={`${h.hour}: ${h.count} errors`}
                  />
                );
              })}
            </div>
