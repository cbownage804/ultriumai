...
import { X, Shield, Mail, Github, Chrome, Key, Loader2, Sparkles } from 'lucide-react';
...
export function AuthConfigPanel({ open, onClose, supabaseConfig, onGenerateAuthPages }: AuthConfigPanelProps) {
  const [providers, setProviders] = useState<AuthProvider[]>([
    ...
    { id: 'google', name: 'Google OAuth', icon: Chrome, enabled: false, description: 'Sign in with Google account' },
    { id: 'github', name: 'GitHub OAuth', icon: Github, enabled: false, description: 'Sign in with GitHub account' },
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-t border-white/[0.06] shrink-0">
            ...
            <p className="text-[9px] text-white/20 mt-1.5 text-center">
              Generates login, signup, and password reset pages
            </p>
          </div>
        </div>
      )}
