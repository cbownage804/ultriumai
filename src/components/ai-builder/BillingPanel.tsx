...
import { X, Zap, TrendingUp, CreditCard, ArrowUpRight, BarChart3 } from 'lucide-react';
          ...
          <div className="flex-1 overflow-auto p-5 space-y-5">
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className={cn("text-lg font-bold", isLow ? "text-red-400" : "text-white")}>
                  {credits.remaining.toLocaleString()}
                </span>
              </div>
              <div className="h-2 rounded-full bg-white/[0.06] overflow-hidden">
                <div
                  className={cn("h-full rounded-full transition-all duration-500", isLow ? "bg-red-500" : "bg-gradient-to-r from-cyan-500 to-violet-500")}
                  ...
                  {history.map(entry => (
                    <div key={entry.id} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg hover:bg-white/[0.02] transition-colors">
                      <div>
                        ...
                        <div className="text-[9px] text-white/20">{entry.timestamp.toLocaleString()}</div>
                      </div>
            ...
            {/* Upgrade CTA */}
            <button className="w-full flex items-center justify-center gap-2 h-10 rounded-xl bg-gradient-to-r from-cyan-500/20 to-violet-500/20 border border-cyan-500/20 text-cyan-400 hover:from-cyan-500/30 hover:to-violet-500/30 transition-all text-xs font-medium">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isLow
          ? "text-red-400 border-red-500/20 bg-red-500/10 hover:bg-red-500/15"
