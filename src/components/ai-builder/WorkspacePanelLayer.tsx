  ...
  return (
    <>
      ...
      <SafePanel show={pv.showPublishPanel} name="Publish">
        <PublishPanel open={pv.showPublishPanel} onClose={() => ps.setShowPublishPanel(false)} publishedUrl={publishedUrl} previewUrl={previewSlug ? `https://${previewSlug}.apps.ultriumai.com` : hostedPreviewUrl} projectName={project.name} hasFiles={project.files.length > 0} onPublish={handlePublish} files={project.files} deployHistory={persistedDeployHistory} onRollback={async (id: string) => { await rollbackToVersion(project.name, id); }} runSmokeTests={props.runSmokeTests} />
      </SafePanel>
