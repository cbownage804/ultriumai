...
if (typeof console !== 'undefined') {
  devLog.log('%c[ENV] Variables loaded:', 'color:#6ee7b7', ${JSON.stringify(maskedObj)});
}
</script>`);
    }
...
(function(){
  ...
  window.fetch = function(){
    ...
    if (typeof req === 'string' && req.startsWith('/') && !req.startsWith('//')) {
      devLog.log('[Preview] Relative fetch URL "' + req + '" — returning mock response. Connect a backend to enable API calls.');
